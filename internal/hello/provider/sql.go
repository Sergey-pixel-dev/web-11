...
func (p *Provider) SelectRandomHello() (string, error) {
	...
	err := p.conn.QueryRow("SELECT message FROM labs ORDER BY RANDOM() LIMIT 1").Scan(&msg)
	if err != nil {
...
func (p *Provider) CheckHelloExitByMsg(msg string) (bool, error) {
	...
	err := p.conn.QueryRow("SELECT message FROM labs WHERE message = $1 LIMIT 1", msg).Scan(&msg)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return false, nil
		}
...
func (p *Provider) InsertHello(msg string) error {
	_, err := p.conn.Exec("INSERT INTO labs (message) VALUES ($1)", msg)
	if err != nil {
		return err
	}
